...
function normalizeOrder(order = {}, index = 0) {
  ...
  return {
    ...
    source: order.source === "manual" ? "manual" : "store",
...
async function createOne(body = {}) {
  ...
  const order = normalizeOrder({
    ...
    status: "processing",
    payment: String(body.payment || "Cash on delivery").trim() || "Cash on delivery",
    delivery: Math.max(0, Number(body.delivery) || 0),
    city: customer.city,
    customer,
    items,
    note: String(body.note || "").trim(),
    source: body.source === "manual" ? "manual" : "store",
